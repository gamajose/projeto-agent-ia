...
def collect_affected_host(executor: SSHExecutor, environment: EnvironmentType) -> dict[str, Any]:
    ...
    return {
        ...
        "port_6556": _run_with_sudo_fallback(
            executor,
            "ss -lntp 2>/dev/null | grep -E '(:|\\])6556\\b' || true",
            environment,
        ),
        "agent_local_output": _run(
            executor,
            "timeout 15 sh -c 'cat < /dev/null > /dev/tcp/127.0.0.1/6556' 2>&1; echo RC:$?",
            environment,
        ),
        "agent_sample": _run(
            executor,
            "timeout 15 sh -c 'exec 3<>/dev/tcp/127.0.0.1/6556; head -n 30 <&3' 2>&1 || true",
            environment,
        ),
        "firewall": _run_with_sudo_fallback(
            executor,
            "(firewall-cmd --list-all 2>/dev/null || nft list ruleset 2>/dev/null || iptables -S 2>/dev/null) | grep -E '6556|check.?mk' || true",
            environment,
        ),
        ...
        "recent_agent_logs": _run_with_sudo_fallback(
            executor,
            "journalctl --no-pager -n 120 -u check-mk-agent.socket -u check_mk.socket -u xinetd 2>/dev/null || true",
            environment,
        ),
...
def discover_monitor(executor: SSHExecutor, environment: EnvironmentType) -> dict[str, Any]:
    ...
    data: dict[str, Any] = {
        "docker": _run_with_sudo_fallback(
            executor,
            "docker info --format '{{json .ServerVersion}}' 2>/dev/null || docker info 2>&1 | head -n 30",
            environment,
        ),
    ...
    for container in containers:
        ...
        data["container_details"].append(
            {
                "container": container,
                "inspect": _run_with_sudo_fallback(
                    executor,
                    f"docker inspect {qname} --format 'StartedAt={{{{.State.StartedAt}}}} RestartCount={{{{.RestartCount}}}} OOMKilled={{{{.State.OOMKilled}}}} ExitCode={{{{.State.ExitCode}}}}'",
                    environment,
                ),
                "sites": _run_with_sudo_fallback(executor, f"docker exec {qname} omd sites 2>&1 || true", environment),
                "events": _run_with_sudo_fallback(
                    executor,
                    f"docker events --since 24h --until 0s --filter container={qname} 2>/dev/null | tail -n 100 || true",
                    environment,
                ),
                "logs": _run_with_sudo_fallback(
                    executor,
                    f"docker logs --since 24h --tail 150 {qname} 2>&1 || true",
                    environment,
                ),
            }
        )
    ...
    for detail in monitor_data.get("container_details", []):
        ...
        for site in _parse_sites(detail["sites"]["stdout"]):
            ...
            item: dict[str, Any] = {
                "container": container,
                "site": site,
                "omd_status": _run_with_sudo_fallback(
                    executor,
...
SAFE_REMEDIATION_PATTERNS = [
    ...
    re.compile(
        r"^docker\s+exec\s+([A-Za-z0-9_.-]+)\s+omd\s+stop\s+([A-Za-z0-9_-]+)\s*&&\s*"
        r"docker\s+exec\s+\1\s+omd\s+start\s+\2$"
    ),
]
...
def _default_validation(command: str) -> str:
    ...
    if not match:
        match = re.search(
            r"docker\s+exec\s+([A-Za-z0-9_.-]+)\s+omd\s+stop\s+([A-Za-z0-9_-]+)\s*&&",
            command,
        )
    if match:
...
def _execute_remediations(
    analysis: dict[str, Any],
    affected: SSHExecutor,
    monitor: SSHExecutor,
    environment: EnvironmentType,
) -> list[dict[str, Any]]:
            ...
            results.append(
                {
                    ...
                    "target": target,
                    "status": "blocked",
                    "reason": decision.reason if not decision.allowed else "Comando fora da lista segura.",
                }
            )
            continue
...
def run_full_diagnosis(
    *,
    ...
    incident_id = save_incident(
        ...
        normalized_output=validated_output,
        evidence=evidence,
        analysis=analysis,
    )
